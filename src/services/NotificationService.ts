...
export async function buildNotifications(): Promise<NotifItem[]> {
  ...
  if ((settings.peringatanTelat || settings.peringatanNoOut) && reportRes.status === 'fulfilled') {
    ...
    for (const r of records) {
      ...
      if (settings.peringatanTelat && masuk === 'T' && r.start) {
        raw.push({
  ...
  if (checkRes.status === 'fulfilled') {
    const { absent, presence } = checkRes.value.data.data;
    ...
    if (absent.type === 'IN' && isWorkday && serverTime >= presence.onstart && serverTime <= presence.endstart) {
      raw.push({
    ...
    if (settings.peringatanNoOut && absent.type === 'OUT' && isWorkday
