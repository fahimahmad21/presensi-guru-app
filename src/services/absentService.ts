...
import { AbsentInfo, AbsentCheck, DataTableResponse, AbsentHistoryItem, AbsentHistoryDetail, AbsentReportItem } from '../types';
...
export function getAbsentDetail(id: string) {
  return apiClient.get<{ status: boolean; message: string; data: AbsentHistoryDetail; info: string }>(`/absent/history/${id}`);
}
